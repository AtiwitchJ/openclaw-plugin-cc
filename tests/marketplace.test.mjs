...
test("marketplace.json exists and is valid JSON", () => {
  ...
  assert.equal(parsed.plugins[0].name, "openclaw");
});
...
test("marketplace.json entry has matching source dir and plugin.json", () => {
  ...
  assert.equal(json.name, "openclaw");
...
test("every command file exists", () => {
  ...
  const cmdDir = path.join(ROOT, "plugins", "openclaw", "commands");
...
test("rescue subagent exists", () => {
  const agentFile = path.join(ROOT, "plugins", "openclaw", "agents", "openclaw-rescue.md");

...
const PLUGIN_ROOT = path.join(ROOT, "plugins", "openclaw");
...
test("codex plugin manifest exists and is host-neutral", () => {
  ...
  assert.equal(parsed.interface.displayName, "OpenClaw");
...
test("codex marketplace entry points at the plugin source", () => {
  ...
  assert.equal(parsed.name, "agents-plugin-cc-openclaw");
  const entry = parsed.plugins.find((plugin) => plugin.name === "openclaw");
  assert.ok(entry, "Missing openclaw entry");
  assert.equal(entry.source.source, "local");
  assert.equal(entry.source.path, "./plugins/openclaw");

...
export function renderTaskResult({ text, failureMessage, reasoningSummary }, { title, jobId = null, write = false, agentName = "OpenClaw" } = {}) {
  const lines = [
    `# ${title ?? "Kilo task"}`,
...
export function renderReviewResult(text, { reviewLabel = "Review", targetLabel = "working tree", sessionId = null, agentName = "OpenClaw" } = {}) {
  const lines = [
    `# ${reviewLabel}`,
    `- Target: ${targetLabel}`,
...
export function renderStoredJobResult(job, storedJob, { agentName = "OpenClaw" } = {}) {
...
export function renderTransferResult({ threadId, resumeCommand, agentName = "OpenClaw" }) {
  return [
    `# ${agentName} transfer`,
